computeLasso <- function(observedX, observedY, bandwidth, evalPoints, degree){
  
  
  lassoDerivatives <- matrix(nrow=length(evalPoints), ncol=degree+1)
  lambdas <- c()
  ...
  for(i in 1:length(evalPoints)){
    ...
    #Run CV to get best lambda
    PRESSCV <- PRESS.cv(observedX, observedY, X, weights)
    
    
    lambdas <- c(lambdas, PRESSCV$minLambda)
    lassoDerivatives[i,] <- PRESSCV$LassoCoef
  }
  
  LassoData <- list(lassoDerivatives = lassoDerivatives, lambdas=lambdas)
  return(LassoData)
}

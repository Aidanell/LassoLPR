...
ComputeLocPolyMethods <- function(x, y, trueEquation, drv, bandwidth){
  ...
  NWFit <- KernSmooth::locpoly(x, y, degree = drv, drv = drv, bandwidth = bandwidth, gridsize = 401)
  NWError <- findError(trueEquation, NWFit$x, NWFit$y)
  NWIntError <- findInteriorError(trueEquation, NWFit$x, NWFit$y)
  
  LLFit <- KernSmooth::locpoly(x, y, degree = drv+1, drv = drv, bandwidth = bandwidth, gridsize = 401)
...
LPRSmoothingEpan <- function(
  ...
  for(i in 1:length(evalPoints)){
    ...
    LassoCoef <- RegularizationLPR(X, y, weights, alpha=1) #Lasso-LPR
    RidgeCoef <- RegularizationLPR(X, y, weights, alpha=0) #Ridge-LPR
  ...
  for(i in 1:4){
    data <- list("LassoFit" = lassoFits[[i]], "NWFit" = ErrList[[i]]$NWFit,
                 ...
                 "LassoIntError" = ErrList[[i]]$LassoIntError, "RidgeIntError"=ErrList[[i]]$RidgeIntError,
                 "NWIntError"=ErrList[[i]]$NWIntError, "LLIntError"=ErrList[[i]]$LLIntError)
    lassoData[[i]] <- data
  }
  return(lassoData)
}
...
#Test Run
testLasso <- LPRSmoothing(equation = bimodal,
                          left = 0,
                          right = 1,
                          numTerms = 10,
                          sigma = 0.1,
...
testLassoEpan <- LPRSmoothingEpan(equation = sine,
                          left = 0,
                          right = 1,
                          numTerms = 10,
                          sigma = 0.5,

...
smoothedLambas <- lowess(UnsmoothedData[[1]]$EvalPoints, UnsmoothedData[[5]])$y
...
par(mfrow=c(4,2))
for(i in 0:2){
  plotLassoObject(UnsmoothedData, deriv=i)
  plotLassoObject(SmoothedData, deriv=i)
}
#Plotting lambda magnitudes
plot(UnsmoothedData[[1]]$EvalPoints, UnsmoothedData[[5]], main="Lambda Magnitude")
plot(SmoothedData[[1]]$EvalPoints, SmoothedData[[5]], main="Lambda Magnitude")

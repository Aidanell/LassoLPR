...
#This file takes LPR3DSmoothing.R and runs in many times and compiles all
#iterations into a single object to analyze performance of the method.
#Data must be cleaned

runAnIteration <- function(){
...
#Runs Simulation
CompleteSimulationData <- vector(mode='list', length=numIterations)
for(i in 1:numIterations){
  CompleteSimulationData[[i]] <- runAnIteration()
  print(paste(i, " iterations completed"))
}



#Analyzes how often each derivative performed better than locpoly
LassoPerformance <- rep(0, 10)
for(i in 1:10){
  
  TimesBetter <- 0
  for(j in 1:length(CompleteSimulationData)){
    TimesBetter <- TimesBetter + as.integer(CompleteSimulationData[[j]]$ErrorDf[i,3] == TRUE)
  }
  LassoPerformance[i] <- TimesBetter
}
LassoPerformance <- LassoPerformance / numIterations #Make percentage

LassoPerformance

CompleteSimulationData[[100]]$ErrorDf

save(CompleteSimulationData, file="3DpeakSimData.RData")
